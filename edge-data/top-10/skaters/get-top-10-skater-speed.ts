...
export async function getTop10SkaterSpeed({ position = "all", sort = "max", seasonId, gameTypeId }: GetTop10SkaterSpeedParams): Promise<SkaterSpeedEntry[]> {
    const url = `${BASE}/skater-speed-top-10/${position}/${sort}/${seasonId}/${gameTypeId}`;
    return fetchJsonWithRetries<SkaterSpeedEntry[]>(url);
}